...
class Editor {
    ...
    addEntity() {
        ...
        if (this.portals[sE.colour] && this.portals[sE.colour].length >= 2) {
            return;
        }
    ...
    refreshPortals() {
        ...
        for (let i = 0; i < this.course.entities.length; i += 1) {
            ...
            if (e.name === 'portal') {
                if (this.portals[e.colour]) {
                    this.portals[e.colour].push(e);
                } else {
                    this.portals[e.colour] = [e];
                }
            }
        }

        [ 'red', 'yellow', 'blue' ].forEach(colour => {
            if (this.portals[colour] && this.portals[colour].length === 2) {
                const first = this.portals[colour][0];
                const second = this.portals[colour][1];

                first.toX = second.x + HALF;
                first.toY = second.y + HALF;
                second.toX = first.x + HALF;
                second.toY = first.y + HALF;
            }
        });
    }
