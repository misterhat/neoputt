...
class Editor {
    ...
    cursorDown() {
        if (!this.cursorWasDown) {
            ...
            if (this.selectedEntity !== -1) {
                ...
                this.undoHistory.push({ tiles: clone(this.course.tiles) });
            }
        }
    ...
    saveState() {
        ...
        sessionStorage.setItem('neoputt-editor', lzwcompress.pack(this.state));
    }

    loadState() {
        let state = sessionStorage.getItem('neoputt-editor');
        ...
        if (state.name === this.course.name) {
            const lastCourse = state.undoHistory[state.undoHistory.length - 1];

            if (lastCourse.tiles &&
                deepEqual(lastCourse.tiles, this.course.tiles)) {
                return;
            }
