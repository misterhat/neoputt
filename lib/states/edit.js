...
const getToken = require('../xhr/get-token');
const h = require('hyperscript');
const postCourse = require('../xhr/post-course');
const tiles = require('../tiles');
const validate = require('../validate');
const xhr = require('xhr');

const BUTTON = 'button.neoputt-button';
const CHECK = emoji['heavy_check_mark'];
const MUL_X = emoji['heavy_multiplication_x'];
const NBSP = String.fromCharCode(160);
const THROTTLE = config.coursePostThrottle * 1000;

function toggleProperty(obj, prop, el, toggle) {
...
class EditState {
    constructor(game, course = new Course()) {
        ...
        this.editor = new Editor(new Course(Object.assign({}, course)));
    ...
    saveToLocal() {
        if (!this.name.value.length) {
            this.header.update('pick a course name!', '#b00');
            return;
        }
    ...
    setFill(toggle) {
        toggleProperty(this.editor, 'fill', this.fill, toggle);
    }

    setGrid(toggle) {
        toggleProperty(this.editor, 'grid', this.grid, toggle);
    }
    ...
    uploadCourse() {
        ...
        postCourse(this.editor.course, (err, body, res) => {
            if (err) {
