...
class EditState {
        ...
        this.wrap = h('div', { style: { 'background-color': '#aaa' } },
            h('.neoputt-toolbox', { style: { height: fToolHeight } },
                h('.neoputt-tools',
                    ...
                    this.useClear),
                h('.neoputt-tiles', { style: { width: tilesWidth } },
    ...
    tick() {
        this.cellX = Math.floor(this.cursor.x / config.tileSize);
        this.cellY = Math.floor(this.cursor.y / config.tileSize);
        this.cellX = this.cellX < 0 ? 0 : this.cellX;
        this.cellY = this.cellY < 0 ? 0 : this.cellY;

        this.coords.innerHTML = `(${('00' + this.cellX).slice(-2)}, ` +
            `${('00' + this.cellY).slice(-2)})`;
    }
    ...
    draw() {
        ...
        if (this.cursor.down) {
            this.editor.paint(this.cellX, this.cellY);
        }
    }
