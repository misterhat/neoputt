...
class FieldState {
        ...
        if (this.type === 'lobby') {
            ...
            this.chatInput.addEventListener('keyup', e => {
                ...
                if (e.keyCode === 13 && value.length) {
                    ...
                    let command = value.match(/^::(\w+)/);

                    if (command) {
                        command = command[1];
                        this.lobby.command(command,
                            value.replace(`::${command}`, ''));
                        return;
                    }

                    this.lobby.say(value);
                }
            });
        }
    ...
    tick() {
        const active = this.type === 'lobby' ? this.lobby.active : true;
    ...
    draw() {
        ...
        this.painter.drawEntities(this.entities);
